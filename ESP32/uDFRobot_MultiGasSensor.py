...
class DFRobot_MultiGasSensor(object):
    ...
    def __init__(self, bus, Baud):
        if bus != 0:
            self.i2cbus = SoftI2C(scl=Pin(16), sda=Pin(17))
            # print(f"BUS : {self.i2cbus.scan()}")
            self.__uart_i2c = I2C_MODE
        else:
            self.ser = serial.Serial(
                "/dev/ttyAMA0", baudrate=Baud, stopbits=1, utimeout=0.5
            )
    ...
    def analysis_all_data(self, recv):
        ...
        if self.gastype == "O2":
            ...
            """
            I follow the calculation from DFRobot_MultiGasSensor.cpp
            """
        ...
        self.temp = (
            1 / (1 / (273.15 + 25) + 1 / 3380.13 * (math.log(Rth / 10000))) - 273.15
        )
...
class DFRobot_MultiGasSensor_I2C(DFRobot_MultiGasSensor):
    ...
    def data_is_available(self):
        ...
        *
        ...
        sendbuf[7] = 0x00
        sendbuf[8] = fuc_check_sum(sendbuf, 8)
        self.write_data(0, bytearray(sendbuf), 9)
        utime.sleep(0.1)
        self.read_data(0, recvbuf, 9)
    ...
    def write_data(self, reg, data, length):
        ...
        while 1:
            try:
                self.i2cbus.writeto_mem(self.__addr, reg, bytearray(data))
                return
...
class DFRobot_MultiGasSensor_UART(DFRobot_MultiGasSensor):
    ...
    def read_data(self, reg, data, length):
        global recvbuf
        utimenow = utime.utime()
        i = 0
        while (utime.utime() - utimenow) <= 2:
